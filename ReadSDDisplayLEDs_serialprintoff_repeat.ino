 ...
 *  Used here are the WWS2811 leds. They come in groups of 50, here are call LED strings
 *  
 ...
 *  2) Doesn't echo data to the Serial port. Runs only once, in the setup section.
 *  3) Loops, Runs in the loop section.

Some notes:
 - Required to use a Arduino with larger sram or ram.  the ARDUINO UNO memory is too small.  Can't run both the SD card and LED Libs.
 - For some reason, There is about 500 character max per read call. I had to buffer per string.  
 - the SD card format:
    - Each line defines the LED intensity.
    - each line defines an individual string (1 to 7) 
